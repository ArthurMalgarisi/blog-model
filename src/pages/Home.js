...
const Home = ({ setActive, user, active }) => {
  ...
  const updateState = (docSnapshot) => {
    ...
    if (!isCollectionEmpty) {
      ...
      toast.info("Não há mais blog para exibir");
      setHide(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Tem certeza de que deseja excluir esse blog?")) {
      try {
        setLoading(true);
        ...
        <div className="row mx-0">
          ...
          <div className="col-md-8">
            ...
            {blogs.length === 0 && location.pathname !== "/" && (
              <>
                <h4>
                  Nenhum blog encontrado com a palavra-chave de pesquisa:{" "}
